...
IRrecv::IRrecv(int pin_number, volatile uint8_t *DDR_reg, volatile uint8_t *PIN_reg)
{
  irparams.pin_number = pin_number;
  irparams.blinkflag = 0;
  irparams.DDR_reg = DDR_reg;
  irparams.PIN_reg = PIN_reg;
}
...
void IRrecv::enableIRIn() {
  ...
  // set pin modes
  *irparams.DDR_reg &= ~(1<<irparams.pin_number);
}
...
ISR(TIMER_INTR_NAME)
{
  TIMER_RESET;
  uint8_t irdata = 0;
  if((*irparams.PIN_reg&(1<<irparams.pin_number)) == (1<<irparams.pin_number))
	irdata = 1;
  else
	irdata = 0;
...
int IRrecv::decode(decode_results *results) {
  ...
  /* 
   * Следующие алгоритмы не используются для экономии места т.к. у меня нет таких пультов.
   * При необходимости раскомментировать нужные алгоритмы
   * 
   */
